...
import type { User, Credential, Session } from "../models/auth";
...
authRouter.post("/register/verify", async (c) => {
  ...
  const challengeRow = db.prepare("SELECT * FROM temp_challenges WHERE user_data LIKE ?").get(`%${body.expected.userID}%`) as { id: string; challenge: string; user_data: string } | undefined;
  ...
  if (verification.verified && verification.registrationInfo) {
    ...
    c.header("Set-Cookie", `session=${sessionId}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${7 * 24 * 60 * 60}`);
...
authRouter.get("/login/options", async (c) => {
  ...
  const options = await generateAuthenticationOptions({
    ...
    allowCredentials: users.flatMap(user => {
      const creds = db.prepare("SELECT credential_id FROM credentials WHERE user_id = ?").all(user.id) as { credential_id: string }[];
...
authRouter.post("/login/verify", async (c) => {
  ...
  const challengeRow = db.prepare("SELECT * FROM temp_challenges ORDER BY created_at DESC LIMIT 1").get() as { id: string; challenge: string } | undefined;
  ...
  const credential = db.prepare("SELECT * FROM credentials WHERE credential_id = ?").get(credId) as Credential | undefined;
  ...
  if (verification.verified) {
    db.prepare("UPDATE credentials SET counter = ? WHERE credential_id = ?").run(verification.authenticationInfo.newCounter, credId);
