...
appointmentsRouter.put("/:id", async (c) => {
  ...
  const specialty = body.specialty !== undefined ? body.specialty : (existing as Appointment).specialty;
  ...
  const illnessId = body.illnessId !== undefined ? body.illnessId : (existing as Appointment).illnessId;
...
function syncToDuckDb(appointment: Appointment): void {
  queryDuckDb(
    `INSERT INTO appointment_history VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
    [
      appointment.id, 
