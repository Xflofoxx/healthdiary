...
prescriptionsRouter.put("/:id", async (c) => {
  ...
  const frequency = body.frequency !== undefined ? body.frequency : (existing as Prescription).frequency;
  ...
  const illnessId = body.illnessId !== undefined ? body.illnessId : (existing as Prescription).illnessId;
...
function syncToDuckDb(prescription: Prescription): void {
  queryDuckDb(
    `INSERT INTO prescription_history VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
    [
      prescription.id, 
